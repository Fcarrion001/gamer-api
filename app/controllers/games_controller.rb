...
class GamesController < OpenReadController
  ...
  def indexapi
    ...
    @games = HTTP.headers(:accept => 'application/json', 'user-key' => user_key)
              .get("https://api-2445582011268.apicast.io/games/?fields=*&filter[release_dates.platform][eq]=48&filter[popularity][gte]=8&filter[storyline][exists]=true&filter[first_release_date][gte]=1504144194000&limit=50&scroll=1").to_s
              # binding.pry
              p @games
    render json: @games
  end
...
# used to add it to the wanted_games table.
  ...
  def game_params
    params.require(:game).permit(:game_name, :platform, :release_date, :api_id,
                                 :storyline, :summary, :screenshot, :url,
                                 :totalAdds, :cloudinary_id)
  end
end
